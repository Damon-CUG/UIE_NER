...
from predict5 import do_pre, entites
import time
...
class NameExtract(object):
    ...
    Return: 人名， flag
        人名： string
        flag： int: 0，无确切的名字；1，有人名；2.不恰当的名字； 3.骂人的话
                   [##### old: flag： int: 0，无确切的名字；1，有人名；2.不恰当的名字； 3.姓； 4.骂人的话]
    ...
    def name_extract(self, text):
        ...
        # text_1_prompt = name2test(text_1)
        # print(text_1_prompt)
        ...
        res_name = entites(res_span, path_in)
        res_name = res_name[0]
        print(res_name) #['风云']
        # 5）后处理
        # a）flag = 0, 无确切的名字
        if res_name == 'None':
            return '', 0
        # if res_name in self.dict_name_uncertain:
        #     retubrn res_name, 0
        # b）flag = 2, 不恰当的名字
...
def nametest_f2f():
    ...
    raw_name = []
    count = 0
    path_in = config['test_path']
    path_out = path_in.replace('.txt', '_res_he.txt')
    ...
    for line in f_in.readlines():
        if line == None or line == '\n' or len(line) == 0:
            continue
        line = line.strip()
        raw_name.append(line)
        test = {"content": "可以告诉我你的名字吗？{}".format(line), "result_list": [], "prompt": "人名"}
        f_out.write(str(test) + '\n')
        count += 1
    f_in.close()
    f_out.close()

def nametest_t2f(text):
    """处理测试集样本为指定格式"""
    path_gen = config['test_gen_path']
    f_gen = open(path_gen, 'w', encoding='utf-8')
    test = {"content": "可以告诉我你的名字吗？{}".format(text), "result_list": [], "prompt": "人名"}
    f_gen.write(str(test).replace("'", '"') + '\n')
    f_gen.close()
    return str(test)
...
def predict_f2f(path_in):
    gs = NameExtract()
    path_out = path_in.replace('.txt', '_res_p4.txt')
    f_in = open(path_in, 'r', encoding='utf-8')  # './data/name_text_sample_order02.txt'
    f_out = open(path_out, 'w', encoding='utf-8')
    count = 1
    for line in f_in.readlines():
        if line == None or line == '\n' or len(line) == 0:
...
def predict_t2f(text):
    gs = NameExtract()
    path_out = './data/predict_t2f_res.txt'
    f_out = open(path_out, 'w', encoding='utf-8')
    res, flag = gs.name_extract(text)
    f_out.write(text + '   RESULT: ' + str(res) + '   FLAG: ' + str(flag) + '\n')
    f_out.close()
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    res, flag = gs.name_extract(s9)
    print(time.time() - start_time)
    print('res: ', res)
    print('flag: ', flag)

    start_time = time.time()
    res, flag = gs.name_extract(s10)
    print(time.time() - start_time)
    print('res: ', res)
    print('flag: ', flag)

    start_time = time.time()
    res, flag = gs.name_extract(s11)
    print(time.time() - start_time)
    print('res: ', res)
    print('flag: ', flag)

    start_time = time.time()
    res, flag = gs.name_extract(s12)
    print(time.time() - start_time)
    print('res: ', res)
    print('flag: ', flag)
